<template>
    <div class="sbform">
        ...
        <el-form :model="brands" :rules="rules" ref="brands" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item>
                <el-button type="primary" @click="submitForm('brands')">更新</el-button>
...
<script>
    export default {
        ...
        created(){
            ...
            axios.get(`http://127.0.0.1:8000/admin/shop/brands/${id}`).then(res=>{
                this.brands = res.data.data.brand
                this.brands.is_show = !!res.data.data.brand.is_show

                this.fileList =[{name:this.brands.name,url:this.brands.image}]
        ...
        methods: {
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put(`http://127.0.0.1:8000/admin/shop/brands/${this.brands.id}`,this.brands).then(res=> {
                            this.$message({
                                showClose: true,

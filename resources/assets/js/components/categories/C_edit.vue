<template>
    <div class="edit">
        ...
        <el-form :model="categories" :rules="rules" ref="categories" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item>
                <el-button type="primary" @click="submitForm('categories')">更新</el-button>
    ...
    export default {
        ...
        methods:{
            getData(){
                ...
                axios.get(`http://127.0.0.1:8000/admin/shop/categories/${id}`).then(res=>{
                    console.log( this.categories)
                    this.categories = res.data.data.category
                    this.categories.is_show = !!res.data.data.category.is_show
                    this.fileList =[{name:this.categories.name,url:this.categories.image}]
                })
            },
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put(`http://127.0.0.1:8000/admin/shop/categories/${this.categories.id}`, this.categories).then(res => {
                            this.$message({
                                showClose: true,
